 ...
 * <p><br>
 * Invokes a <i>TheTVDB.com</i> remote API route and verifies that the invocation was successful. Prints the description text
 * of this assertion to the standard console-out stream. For non-void routes the object returned by the remote API will be
 * converted to a proper String representation and will be printed to the standard console-out too.
 * <pre>{@code
...
public abstract class AbstractIntegrationTestAssert<T> extends AbstractAssert<AbstractIntegrationTestAssert<T>, T> {
    ...
    /**
     * Invoke the actual <i>TheTVDB.com</i> API route and verify that it did not throw any exception. Print description text
     * and API response (if available) to the standard console-out.
     */
    public void doesNotThrowAnyException() {
        final Function<Object, String> toString = obj -> APIUtil.toString(() -> obj);
        final Function<String, String> trimOutput = s -> s.length() > MAX_OUTPUT_CHARACTERS ? s.substring(0, MAX_OUTPUT_CHARACTERS).concat("...") : s;
    ...
    /**
     * Prints the description text and the given response String to the standard console-out stream, applying some formatting first.
     *
